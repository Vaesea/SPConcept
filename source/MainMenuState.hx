...
class MainMenuState extends FlxState
{
	...
	override public function update(elapsed:Float)
	{
        ...
        else if (FlxG.keys.justPressed.W) // The state will be changed to a level select once that is actually finished. For now it's a key where you go to PlayState quickly.
        {
            FlxG.sound.music.stop();
    ...
    function clickPlay() // Starts the timer that switches to PlayState, but first it has to remove buttons and play titleEnd.ogg while also stopping the music!
    {
        FlxG.sound.music.stop();
    ...
    function completeTimer(timer:FlxTimer) // Switches to PlayState. Simple.
    {
        FlxG.switchState(new PlayState());
    }
    ...
    function clickClose() // Closes game. Simple.
    {
        System.exit(0);
    }
}
